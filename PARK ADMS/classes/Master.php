...
Class Master extends DBConnection {
	...
	function delete_vehicle(){
		...
		function save_customer(){
			...
			$data = "";
			foreach ($_POST as $k => $v) {
				if (!in_array($k, array('CustomerID'))) {
					if (!empty($data)) $data .= ",";
					$v = htmlspecialchars($this->conn->real_escape_string($v));
					$data .= " `{$k}`='{$v}' ";
				}
			...
			if (empty($CustomerID)) {
				...
				$sql = "UPDATE `customers` SET {$data} WHERE `CustomerID` = '{$CustomerID}'";
			}
			$save = $this->conn->query($sql);
			if ($save) {
				$pid = !empty($CustomerID) ? $CustomerID : $this->conn->insert_id;
				$resp['pid'] = $pid;
				$resp['status'] = 'success';
				if (empty($CustomerID))
					$resp['msg'] = "New Customer details saved successfully.";
				else
	...
	function delete_customer(){
		extract($_POST);
		$del = $this->conn->query("DELETE FROM `customers` WHERE `CustomerID` = '{$id}'");
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success', " Customer successfully deleted.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function get_customers(){
		$customers = array(); // Retrieve customers from your database
	...
	function get_customer_details(){
		$customer_id = $_GET['id']; // Retrieve customer details from your database based on the customer_id
		// Construct a JSON response
		$customer_details = array(); // Fetch customer details from your database
		$response = array(
			'status' => 'success',
...
switch ($action) {
	...
	break;
	case 'get_customers':
        echo $Master->get_customers();
        break;

    case 'get_customer_details':
        echo $Master->get_customer_details();
        break;
	default:
		// echo $sysset->index();
		break;
